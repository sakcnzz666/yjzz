...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!env.DB) {
      return new Response("未绑定 D1 数据库（变量名应为 DB）", { status: 500 });
    }
...
(function () {
  ...
  function refreshUI() {
    ...
    if (needFile) {
      ...
      input.setAttribute("accept", isVideo
  ...
  $("#genBtn").addEventListener("click", async () => {
    ...
    if ((mode === "image-upload" || mode === "video-upload")) {
      ...
      if (file.size > 5 * 1024 * 1024) { alert("文件超过 5MB，请压缩后再上传"); return; }
    }
    ...
    try {
      ...
      btn.disabled = false;
      btn.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> 生成追踪代码';
    }
  });
  ...
  $("#burnBtn").addEventListener("click", () => {
    if (!confirm("查询后将立即删除该 ID 下的所有记录，确认继续？")) return;
    doQuery(true);
  });
...
async function renderAdmin(request, env) {
  ...
  const targetsResult = await env.DB.prepare(
    "SELECT id, image_type, image_url, creator_ip, creator_ua, creator_webrtc_ips, creator_fingerprint FROM targets ORDER BY created_at DESC LIMIT 500"
  ).all();
    ...
    logRows += `<tr class="border-b border-slate-100 hover:bg-slate-50/70 align-top">
      ...
      <td class="p-2 text-[11.5px] max-w-[200px]">
        <div class="flex flex-wrap items-center gap-1">
          <span class="ip-addr">${esc(r.ip || "")}</span>${flag}${localBadge}
  ...
  for (const [id, info] of Object.entries(targetMap)) {
    ...
    if (info.creator_webrtc_ips) {
      try {
        ...
        if (Array.isArray(arr) && arr.length) {
          webrtcHtml = arr.map(ip =>
            `<span class="ip-addr mr-1">${esc(ip)}</span>` +
    ...
    creatorRows += `<tr class="border-b border-slate-100 hover:bg-slate-50/70 align-top">
      ...
      <td class="p-2 text-[11.5px] max-w-[220px]">${webrtcHtml}</td>
...
<body>
  <div class="box">
    <div class="flex items-center gap-3 mb-6">
      ...
      <div>
        <h1 class="text-[17px] font-bold leading-tight">后台登录</h1>
        <p class="text-[11.5px] text-slate-400 mt-0.5">仅限管理员访问</p>
      </div>
    </div>
    ...
    <div class="mt-6 pt-5 border-t border-slate-100 text-center text-[11px] text-slate-400 leading-relaxed">
      Copyright © 2026 SAK All rights reserved.
    </div>
  </div>
